<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Cuti_model extends CI_Model
{
	
	private static $_table = 'cuti';
    private static $_pk = 'id_cuti';
    
    // public function is_exist($where)
	// {
	// 	return $this->db->where($where)->get(self::$_table)->row_array();
	// }

	public function data($where)
	{
		return $this->db->where($where)->where('id_user',$this->session->userdata('usr_id'))->get(self::$_table)->row_array();
	}

	public function insert($data)
	{
		return $this->db->insert(self::$_table, $data);
	}

	public function update($data, $id)
	{
		return $this->db->set($data)->where(self::$_pk, $id)->update(self::$_table);
	}

	public function delete($id)
	{
		return $this->db->delete(self::$_table, array('id_cuti' => $id));
	}

	 public function get_user()
    {
    	$this->db->select('*');
        $this->db->where('usr_level','Admin');
        return $this->db->get('user')->result_array();
    }

     public function get_data($bulan)
    {
        $this->db->select('*,usr_nama as nama_pem,usr_username as username');
        $this->db->join('user u', 'j.id_user = u.usr_id');
        $this->db->where('id_user',  $this->session->userdata('usr_id'));
        $this->db->like('tanggal', $bulan);
        return $this->db->get('cuti j')->result_array();
    }
}